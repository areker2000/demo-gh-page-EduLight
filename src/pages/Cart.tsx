...
import type { ICoupons } from '../types/localData';
import type { ICartForm } from '../types/form';
...
const FormLable = ({ forID, text }: IFormLabelProps) => {
  return (
    <label
...
const Cart = () => {
  ...
  const getCart = async () => {
    ...
    try {
      ...
      const total = carts.reduce(
        (sum: number, currClass: ICartItem) => sum + currClass.final_total,
        0,
      );
  ...
  const [selectedCoupon, setSelectedCoupon] = useState<ICoupons>(initCoupon);
  ...
  const {
    ...
    mode: 'onBlur',
    defaultValues: { cardNumber: '', cardName: '', email: '', tel: null },
  });
  const onSubmit = (data: ICartForm) => {
    alert('資料已送出');
  };
  ...
  const handleNumberOnly = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const originCursor = input.selectionStart ?? 0;
  ...
  return (
    <>
      ...
      <TeacherModal
        ...
        initVal={Number(selectedItem?.qty) || 1}
        btnText={['修改時數', '回到購物車']}
        selectedId={editId}
      />
    </>
